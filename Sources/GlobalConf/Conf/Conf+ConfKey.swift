...
public extension Conf {
	...
	static func withValue<InjectedKey : ConfKey, T>(_ newValue: InjectedKey.Value, for keyType: InjectedKey.Type = InjectedKey.self, operation: () async throws -> T) async rethrows -> T {
		var newContext = Conf.currentContext
		newContext[InjectedKey.self] = newValue
	...
	@inlinable
	@_unsafeInheritExecutor /* Same as withValue declared in the stdlib (and ServiceContext); because we do not want to hop off the executor at all. */
	static func withValue<InjectedKey : ConfKey, T>(_ newValue: InjectedKey.Value, for keyPath: KeyPath<ConfKeys, InjectedKey.Type>, operation: () async throws -> T) async rethrows -> T {
